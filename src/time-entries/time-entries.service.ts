...
export class TimeEntriesService {
  ...
  async findAll(params: TimeEntryQueryParams): Promise<{ data: TimeEntry[]; total: number; page: number; limit: number }> {
    const { 
      userId,
      caseId,
      billable,
    ...
    if (startDate) {
      ...
      timeEntries = timeEntries.filter(entry => new Date(entry.startTime) >= startDateTime);
    }
    
    if (endDate) {
      const endDateTime = new Date(endDate);
      timeEntries = timeEntries.filter(entry => new Date(entry.startTime) <= endDateTime);
    }
